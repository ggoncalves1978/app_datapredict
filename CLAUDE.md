...
**DataPredict** is a full-stack time series analysis and forecasting web app. Users upload CSV/Excel files and get statistical analysis, seasonal decomposition, stationarity tests, ACF/PACF correlations, multi-model forecasts, and advanced diagnostics.
...
uvicorn main:app --reload --port 8001   # Dev server (use 8001 to avoid port conflicts)
```
...
### Data Flow
1. User uploads CSV/Excel in `Dashboard.tsx` → parsed client-side by PapaParse/XLSX
2. Parsed data stored in Zustand store (`frontend/src/store/useDataStore.ts`)
...
- **`store/useDataStore.ts`** — Zustand global store; holds `dataset`, `secondaryDataset`, all analysis results, and `diagnostics` state slice
- **`pages/`** — One page per analysis type (Dashboard, TrendSeasonality, Stationarity, Correlations, Comparison, Forecast, Diagnostics)
- **`components/layout/`** — Shell (MainLayout, Header, Sidebar)

### Backend (`backend/main.py`)
Single-file FastAPI app (~1400 lines). All endpoints live here:
...
| `POST /api/forecast` | Multi-model forecast with ranking |
...
```
`periodo` must be a parseable ISO date string.

### Forecast Engine
Located in `main.py`, trains 5 models on a train/test split, ranks by MAPE:
...
- **Backend:** FastAPI, Uvicorn, Pandas, Statsmodels, SciPy, sktime
